            ...
            if(data.account){
                ...
                if($idAccount.val().indexOf("@") > 0 ){
                    Dml.fun.showTipsDialog({
                        ...
                        h2: 'We have sent email to your email '+ $idAccount.val() +', please click the link to change password'
                    });
                    $('#jsFindPwdForm')[0].reset();
                    setTimeout(function(){window.location.href = window.location.href;},1500);
                }else{
                    if(data.status == 'success'){
                        $('#jsForgetTips').html("The code has been sent").show();
                        $('#jsInpResetMobil').val($idAccount.val());
                        setTimeout(function(){Dml.fun.showDialog('#jsSetNewPwd')},1500);
                    }else if(data.status == 'failure'){
                        $('#jsForgetTips').html("verification code failed to send!").show();
                    }
                }
            }
        },
...
$(function() {
	...
	$('#jsSenEmailAgin').on('click', function(e){
        ...
        $.ajax({
            ...
            data: {username:zyUname},
             success: function(data){
                 zy_str="The message was sent successfully";
                 //console.log(data)
                 if(data)
                    zy_Countdown();
             },
    ...
    $('.listoptions').find('.more').click(function(){
        ...
        if(!$('.listoptions .cont').hasClass("opened")){
            ...
            $('.listoptions').find('.more').html('Collapse')
        }else{
