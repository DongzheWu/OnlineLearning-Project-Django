...
function comment_toggle(){
	$('.com_child_toggle').live('click',function(){
		 $(this).toggle(function(){
		 	$(this).parent().parent().find('>div:last').css('display','block').find("textarea").focus().val("reply"+$(this).next().html()+":").attr("vu","reply"+$(this).next().html()+":");
		 	...
		 	$(this).parent().parent().find('>div:last').css('display','none').find("textarea").focus().val("reply"+$(this).next().html()+":");
		 }).trigger('click');
	});
...
function submit_comment(){
	...
	var	teacher = "";
		if(is_teacher =='True'){
			teacher = '<span class="grade">Instructor</span>';
		}
	//zhouyi:8-5
	$('#release_com').click(function(){
		...
		if(comment.trim()=='') {
			layer.tips('The content can not be blank!', '.comment-input .btnbox>button', {tips: [2, '#68c8b6']});
			return;
		}
		parent_id = ""
		child_uid = ""
	...
	$('.reply').live("click",function(){
		...
		str_div = '<div class="media"><a href="javascript:void(0);" class="pull-left"><img src="'+picurl+'" class="media-object"></a><div class="media-body"><h4 class="media-heading"><a class="com_child_toggle" href="javascript:;">reply</a><span class="user-name">'+cur_name+'</span>'+teacher+'<span class="time">just a moment ago\n</span><span class="child_uid">'+child_uid+'</span></h4><div class="zy_shou"><p class="t5o">'+html_encode(comment)+'</p><p>More</p></div></div></div>';
		str_div=$(str_div);
		if(comment!=""){
    ...
    $.ajax({
            ...
            success: function(data) {
            	...
            	if(data){
            		...
            		if(page == 1){
            			$('#load_message').html('Write comments!');
            		}else{
