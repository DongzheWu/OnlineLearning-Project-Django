...
class CourseInfoView(LoginRequiredMixin, View):
    def get(self, request, course_id):
        ...
        #get courses that other users have taken
...
class AddComentsView(LoginRequiredMixin, View):
    """
    user add comments function
    """
    def post(self, request):
        if not request.user.is_authenticated:

            return HttpResponse('{"status":"fail", "msg":"The usr has not logged in yet"}', content_type='application/json')
        ...
        if int(course_id) > 0 and comments:
            ...
            return HttpResponse('{"status":"success", "msg":"add success"}', content_type='application/json')
        else:
